    ...
    fn execute_check_transaction_on_state(
        ...
        let events = match &tx {
            NSSATransaction::Public(t) => self
                .state
                .transition_from_public_transaction(t, self.next_block_id()),
...
pub fn produce_new_block_with_mempool_transactions(&mut self) -> Result<(SignedMantleTx, MsgId)> {
    let now = Instant::now();
...
mod tests {
    ...
    async fn transaction_execute_native_transfer() {
        ...
        sequencer
            .execute_check_transaction_on_state(tx)
            .map(|_| ())
            .unwrap();
